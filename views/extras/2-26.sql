DECLARE
  TYPE outputRow IS TABLE OF VARCHAR2(2048);
...
BEGIN
  ...
  FOR row IN (SELECT Строка FROM Dop_2_26) LOOP
    ...
    ELSE
      ...
      FOR currentLength IN REVERSE 1..(FLOOR(len / 2)) LOOP
        ...
        -- <<posLoop>> FOR startPos IN 1..(len - currentLength + 1) LOOP
        &lt&ltposLoop&gt&gt FOR startPos IN 1..(len - currentLength + 1) LOOP
  ...
  DECLARE
    rowOutput VARCHAR2(4096);
    horizontalRule VARCHAR2(4096);
  BEGIN
